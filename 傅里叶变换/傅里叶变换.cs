...
public class FourierTransformScript : TunnelExtensionScriptBase
{
    ...
    public override FrameworkElement CreateParameterControl()
    {
        ...
        var viewModel = CreateViewModel() as FourierTransformViewModel; // Assuming a ViewModel named FourierTransformScriptViewModel
        ...
        // Assuming similar CheckBox controls as the other Fourier script
        mainPanel.Children.Add(CreateCheckBoxControl("应用对数变换", "ApplyLogTransform", viewModel, resources));
        mainPanel.Children.Add(CreateCheckBoxControl("零频率居中", "CenterFFT", viewModel, resources));
        mainPanel.Children.Add(CreateCheckBoxControl("存储元数据", "StoreMetadata", viewModel, resources));
        mainPanel.Children.Add(CreateCheckBoxControl("应用汉宁窗", "ApplyWindow", viewModel, resources));
        mainPanel.Children.Add(CreateCheckBoxControl("增强预览效果", "EnhancePreview", viewModel, resources));
        mainPanel.Children.Add(CreateCheckBoxControl("变换Alpha通道", "TransformAlpha", viewModel, resources));
        mainPanel.Children.Add(CreateCheckBoxControl("增强相位谱对比度", "EnhancePhaseContrast", viewModel, resources));

        return mainPanel;
    }

    private CheckBox CreateCheckBoxControl(string content, string propertyName, object viewModel, ResourceDictionary resources)
    {
    ...
    {
        var viewModel = new FourierTransformViewModel(this);
        return viewModel;
    }
